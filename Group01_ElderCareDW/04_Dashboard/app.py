...
with tabs[6]:
    ...
    if mk.empty:
        ...
        if mk["m10_beds_per_1000_elderly"].isna().all():
            st.error(
                "**M10 (เตียงต่อผู้สูงอายุพันคน) ยังคำนวณไม่ได้** — คอลัมน์ `pop_65plus` ว่างทั้งคอลัมน์ "
                "หน้านี้จึงตอบ BQ1 ได้เพียงครึ่งเดียว: บอกได้ว่ารัฐไหนอุปทานตึง แต่บอกไม่ได้ว่ารัฐไหน "
                "**ยังมีที่ว่างสำหรับเตียงใหม่** เพราะอัตราการเข้าพักต่ำอาจแปลว่าอุปสงค์น้อยจริงก็ได้"
            )
        ...
        note("มุมซ้ายล่าง = เข้าพักต่ำและคู่แข่งคุณภาพต่ำ คือช่องว่างที่น่าสนใจที่สุด "
             "แต่ยืนยันไม่ได้จนกว่าจะมีข้อมูลประชากร · แสดงเฉพาะรัฐที่มีอย่างน้อย 20 แห่ง")
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "m10_beds_per_1000_elderly": st.column_config.NumberColumn(
                    "M10 เตียง/ผู้สูงอายุพันคน", format="%.2f", help="ว่างจนกว่าจะโหลดข้อมูล Census"),
            })
...
with tabs[9]:
    ...
    st.markdown("""
- **`pop_65plus` ว่างทั้งคอลัมน์** → M10 และ BQ1 ยังตอบไม่ครบ
- **มีเพียง 3 จุดเวลา** และห่างกัน 7 ปีระหว่างสองจุดแรก → BQ5 ยังไม่ใช่อนุกรมเวลาจริง
- **ปีที่หายไปในกราฟค่าปรับ** เกิดจากหน้าต่างหมุน 3 ปีของแฟ้ม CMS ไม่ใช่ปีที่ไม่มีการลงโทษ
- **แฟ้ม Penalties ของงวด 2026-07-29 ถูกทิ้งทั้งแฟ้ม** (16,166 แถว) จากบั๊กใน
  `sources.drop_republished_periods()` — ตัวเลขค่าปรับทุกตัวบนแดชบอร์ดนี้จะเปลี่ยนเมื่อแก้บั๊กนั้น
