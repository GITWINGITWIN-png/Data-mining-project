...
import queries as vqueries  # noqa: E402  — ชั้น view ที่ insights.py ใช้
...
@st.cache_data(show_spinner=False)
def load_insights(period_key, states, ownership, chains, exclude_suspect):
    """คำนวณ insight สดจากตัวกรองปัจจุบัน

    `insights.py` เขียนขึ้นบนชั้น view (`queries.py`) ส่วนหน้านี้ใช้ `queries_mpl.py`
    ทั้งสองอ่านคลังไฟล์เดียวกัน จึงใช้ connection เดียวกันได้ ที่ต้องแปลงคือ
    รูปของตัวกรองเท่านั้น — ป้ายกำกับกลุ่มเจ้าของและขนาดเครือของสอง sidebar
    ตรงกันอยู่แล้ว จึงส่งต่อได้ตรง ๆ

    รับพารามิเตอร์เป็น tuple ไม่ใช่ Filters เพราะ `st.cache_data` แฮชค่าอาร์กิวเมนต์
    และแฮช object เองไม่ได้
    """
    f = vqueries.Filters(
        periods=[period_key],
        states=list(states),
        ownership=list(ownership),
        chain_bands=list(chains),
        include_suspect=not exclude_suspect,
    )
    return insights.build(get_connection(), f)
...
with st.sidebar:
    ...
    st.divider()
    st.caption(
        "**ที่มา** CMS Nursing Home Compare (32 งวดรายไตรมาส 2562–2569) · "
        "SNF VBP · US Census ประชากร 65+\n\n"
        "**คลัง** DuckDB · 6 Dimension · 2 Fact · "
        "483,183 + 79,803 แถวข้อเท็จจริง"
    )
...
with tab_overview:
    ...
    answers = pd.DataFrame([
        ...
        ("BQ2", "รูปแบบการถือครองต่างกันอย่างไร",
         "For-profit ต่ำกว่าทุกมิติคุณภาพ และเสี่ยงค่าปรับสูงกว่า 1.56 เท่า"),
        ...
        ("BQ4", "ลาออกเท่าไรจึงเริ่มฉุดคุณภาพ",
         "คุณภาพตกทันทีไม่มีระดับปลอดภัย · อัตราเข้าพักเริ่มตกที่ 40% และพังที่ 65%"),
        ("BQ5", "ฟื้นจาก COVID แล้วหรือยัง",
         "เข้าพักดิ่ง 13 จุดถึงก้นเหว ก.ค. 2564 แล้วใช้ 5 ปีฟื้นเต็ม · คุณภาพยังไม่ฟื้น"),
        ("BQ6", "แห่งใดกำลังเสื่อมและเสี่ยงถูกลงโทษ",
         "คะแนนเสี่ยงจัดอันดับได้จริง ผ่านการทดสอบย้อนหลัง 4 ปี"),
        ("BQ7", "ค่าปรับกระจุกที่ใด",
         "ต่างกันเกือบ 15 เท่าระหว่างรัฐ · ช่วงโควิดปรับถี่ขึ้น 5.5 เท่าแต่เบาลง 3 เท่า แล้วสวนกลับ"),
...
with tab_reco:
    ...
    st.caption(
        "คำนวณสดจากตัวกรองปัจจุบัน — ข้อความกับตัวเลขในนั้นมาจาก query ชุดเดียวกัน "
        "ในคำขอเดียวกัน จึงขัดกันเองไม่ได้ และการขยับตัวกรองจะคำนวณข้อสรุปใหม่ทั้งหมด"
    )

    # เดิมแท็บนี้เป็นลิสต์ข้อความที่พิมพ์ตัวเลขฝังไว้ ตัวเลขจึงไม่ขยับตามตัวกรอง
    # และค้างอยู่ที่ค่าของคลังชุด 4 งวด · ตอนนี้เรียก insights.build() ซึ่งเป็น
    # โมดูลเดียวกับที่ app.py ใช้ และคำนวณทุกตัวเลขจากตัวกรองที่ผู้ใช้ตั้งไว้จริง
    items = load_insights(period_key, S, O, C, exclude_suspect)

    badge = {
        "solid": ("🟢", "หลักฐานหนักแน่น"),
        "tentative": ("🟡", "หลักฐานยังบาง"),
        "blocked": ("🔴", "ยังตอบไม่ได้ภายใต้ตัวกรองนี้"),
    }

    st.caption(
        f"ได้ {len(items)} ข้อภายใต้ตัวกรองนี้ — จำนวนข้อเปลี่ยนได้ เพราะบางข้อ "
        "เปรียบเทียบระหว่างกลุ่ม และการกรองจนเหลือกลุ่มเดียวทำให้ข้อนั้นไม่มีความหมาย "
        "การซ่อนข้อที่ตอบไม่ได้ตรงไปตรงมากว่าการแสดงตัวเลขที่ไม่มีของให้เทียบ"
    )

    for number, ins in enumerate(items, 1):
        icon, label = badge.get(ins.strength, ("", ""))
        with st.container(border=True):
            st.markdown(f"**{number}. {ins.title}**  &nbsp;`{ins.bq}`  &nbsp;<sub>{icon} {label}</sub>",
                        unsafe_allow_html=True)
    ...
    st.info(
        "**หมายเหตุเรื่องหน้าต่างเวลาของค่าปรับ** ข้อที่ว่าด้วย BQ7 ในหน้านี้ใช้ "
        "**ค่าปรับสะสมทุกปีในคลัง** หารด้วยเตียงของงวดล่าสุด ส่วนกราฟในแท็บความเสี่ยง "
        "ใช้ **หน้าต่าง 3 ปีล่าสุด** เพื่อให้เทียบกับที่ CMS เผยแพร่ได้ "
        "ตัวเลขสองที่จึงไม่เท่ากันโดยตั้งใจ ไม่ใช่ความผิดพลาด — "
        "อันดับรัฐที่เข้มที่สุดขยับได้ตามหน้าต่างที่เลือก และนั่นคือข้อค้นพบในตัวมันเอง"
    )

    st.divider()
    st.subheader("ข้อจำกัดที่ต้องบอกไปพร้อมกับข้อเสนอแนะ")
    for item in [
        "**ข้อมูลเครือมีเฉพาะปี 2569** — คอลัมน์เครือเพิ่งเพิ่มเข้ามาปี 2568 "
        "จึงแยกไม่ได้ว่าเครือทำให้บ้านแย่ลง หรือเครือไปซื้อบ้านที่แย่อยู่แล้ว "
        "และค่าปรับของงวดก่อนปี 2566 ชี้ไปที่แถว Unknown ไม่ใช่ “ไม่สังกัดเครือ”",
        "**คะแนนดาวเป็นไม้บรรทัดที่ CMS ปรับเกณฑ์เองระหว่างทาง** — "
        "คะแนนที่ตกจึงปนการเปลี่ยนเกณฑ์กับการเปลี่ยนคุณภาพจริง "
        "ข้อสรุปเรื่องคุณภาพจึงยืนบนจำนวนข้อบกพร่องและค่าปรับเป็นหลัก",
        "**ค่าปรับเป็นหน้าต่างหมุน 3 ปี ไม่ใช่ยอดสะสม** — ค่าปรับ “ลดลง” "
        "ระหว่างงวดได้เองโดยไม่ได้แปลว่าข้อมูลผิด และ CMS แก้ยอดย้อนหลัง "
        "26,176 รายการ (ลดลงสุทธิ $230M) ซึ่งคลังเก็บยอดล่าสุดไว้",
        "**อัตราการลาออกมีเฉพาะยุค 2569** — BQ4 จึงตอบได้เฉพาะภาพตัดขวาง "
        "ไม่ใช่การเปลี่ยนแปลงตามเวลา",
        "**ทุกข้อเป็นความสัมพันธ์ ไม่ใช่เหตุและผล** — คำเตือนรายข้อที่ขึ้นต้นด้วย ⚠️ "
        "ด้านบนระบุตัวแปรที่ยังไม่ได้ควบคุมของข้อนั้น ๆ ไว้แล้ว",
    ]:
        st.markdown(f"- {item}")
