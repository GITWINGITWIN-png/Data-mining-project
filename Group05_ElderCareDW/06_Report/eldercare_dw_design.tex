...
กลุ่มที่ \underline{\hspace{0.35cm}05\hspace{0.35cm}} \quad สมาชิก: \underline{\hspace{3cm}} ,
...
จับคู่ชื่อไฟล์ที่เปลี่ยนไปเรื่อย แล้วต่อกันเป็นอนุกรมเวลาเอง
ไม่มีไฟล์ใดไฟล์หนึ่งที่เปิดแล้วได้ข้อมูลย้อนหลังทันที
\end{keybox}

\begin{warnbox}[title={ชื่อไฟล์มีอย่างน้อย 3 แบบ ไม่ใช่แค่เปลี่ยนเดือน \ok{}}]
เปิด snapshot ปี 2562 เทียบกับปี 2569 จริงแล้วพบว่าไม่ได้ต่างกันแค่เดือน
แต่ต่างกันทั้ง \emph{ระบบตั้งชื่อ}

\vspace{0.4em}
\begin{tabular}{@{}L{3.2cm} L{12.6cm}@{}}
\toprule
\sansall\bfseries ยุค & \sansall\bfseries รูปแบบชื่อไฟล์ \\
\midrule
2562 & \code{ProviderInfo_Download.csv}, \code{Penalties_Download.csv}
       --- ไม่มี \code{NH_} ไม่มีเดือน \\
2569 (ปกติ) & \code{NH_ProviderInfo_Jun2026.csv} --- มี prefix และเดือน \\
2569 (บางส่วน) & \code{4pq5-n9py_2026-07-29_NH_ProviderInfo_Jul2026.csv}
       --- มีรหัสชุดข้อมูลนำหน้าอีกชั้น \\
\bottomrule
\end{tabular}

\vspace{0.5em}
การจับคู่ด้วย \code{glob} ธรรมดาจึงพังแน่นอน ต้องใช้การจับคู่แบบ
\textbf{regex ต่อยุค} หรืออ่านจาก \code{manifest.json} ถ้ามี
\end{warnbox}

\begin{badbox}[title={snapshot ``บางส่วน'' มีอยู่จริง และ ETL ต้องรับมือ \ok{}}]
งวด 2026-08-06 ที่มีขนาดผิดปกติ 3.5 MB เปิดดูแล้ว \textbf{มีเพียง 4 ไฟล์}:
\code{NH_ProviderInfo}, \code{NH_StateUSAverages}, \code{manifest.json}
และคู่มือ --- \emph{ไม่มี} \code{NH_Penalties} เลย

\vspace{0.3em}
ถ้า ETL สมมติว่าทุก snapshot มีครบ 6 ไฟล์ที่ต้องใช้ งวดนี้จะทำให้ท่อพัง
หรือแย่กว่านั้นคือโหลด Fact 2 ได้ศูนย์แถวแบบเงียบ ๆ

\vspace{0.3em}
\textbf{กฎที่ต้องเขียน:} ตรวจรายชื่อไฟล์ก่อนประมวลผลทุกงวด
งวดใดขาดไฟล์ให้ข้ามเฉพาะ Fact ที่เกี่ยวข้อง ไม่ใช่ข้ามทั้งงวด
และบันทึกลงตารางบันทึกการทำงาน
\end{badbox}
...
\begin{badbox}[title={กับดัก: แถวทดสอบนี้ผูกกับ ``งวด'' ห้ามใช้ลอย ๆ}]
\ok{} ตรวจซ้ำแล้วพบว่า \textbf{ค่าเดียวกันนี้ไม่ตรงกันระหว่างสองแหล่ง}
เพราะเป็นคนละงวด

\vspace{0.3em}
\begin{tabular}{@{}L{5.6cm} L{2.6cm} L{2.4cm} L{2.4cm}@{}}
\toprule
\sansall\bfseries แหล่ง & \sansall\bfseries Processing Date &
\sansall\bfseries ผู้พักอาศัย & \sansall\bfseries อัตราเข้าพัก \\
\midrule
API ปลายทาง C (ข้อมูลปัจจุบัน) & 2026-07-01 & 51.6 & 90.53\% \\
CSV \code{NH_ProviderInfo_Jun2026} & 2026-06-01 & 52.4 & 91.93\% \\
\bottomrule
\end{tabular}

\vspace{0.5em}
ETL โหลดจาก \emph{CSV} ไม่ใช่จาก API ดังนั้นการทดสอบต้องคาดหวัง
\textbf{91.93\% สำหรับงวด มิ.ย. 2569} ถ้าเขียนเทสต์ให้เช็ค 90.5\% ลอย ๆ
เทสต์จะแดงทั้งที่ท่อถูกต้อง

\vspace{0.3em}
บทเรียนที่ใหญ่กว่านั้น: \textbf{ค่าทดสอบทุกตัวต้องระบุงวดกำกับเสมอ}
และนี่คือหลักฐานตรง ๆ ว่าปลายทาง C ใช้ ``ตรวจความถูกต้องหลัง Load''
ได้เฉพาะกับงวดล่าสุดเท่านั้น
\end{badbox}
...
\begin{goodbox}[title={หาสาเหตุที่เรียกไม่สำเร็จได้แล้ว --- Census บังคับใช้ API key}]
\ok{} อาการ ``คืนค่าว่าง'' ไม่ใช่พารามิเตอร์ผิดหรือปีไม่มีข้อมูล แต่เป็นเพราะ
Census ตอบกลับด้วย \textbf{HTTP 302} ชี้ไปที่
\code{https://api.census.gov/data/missing_key.html} ซึ่งเป็นหน้า HTML
ถ้า \code{curl} ไม่ได้ใส่ \code{-L} จะได้ body ว่างเปล่าโดยไม่มี error
--- ตรงกับอาการที่บันทึกไว้ทุกประการ

\vspace{0.3em}
\textbf{วิธีแก้:} ขอคีย์ฟรีที่ \code{api.census.gov/data/key_signup.html}
แล้วต่อ \code{\&key=...} ท้าย URL เก็บคีย์ไว้ในตัวแปรสภาพแวดล้อม
ห้าม hard-code ลงในโค้ดที่ commit
\end{goodbox}

\begin{keybox}[title={ตัวแปร 12 ตัวที่ต้องบวกกันเพื่อให้ได้ ``ประชากร 65+''}]
ยืนยันชื่อตัวแปรจาก \code{groups/B01001.json} แล้ว \ok{} (metadata ไม่ต้องใช้คีย์)

\vspace{0.3em}
\textbf{ชาย:} \code{B01001_020E} (65--66), \code{021E} (67--69),
\code{022E} (70--74), \code{023E} (75--79), \code{024E} (80--84),
\code{025E} (85+)

\vspace{0.3em}
\textbf{หญิง:} \code{B01001_044E} ถึง \code{049E} ตามช่วงอายุเดียวกัน
\end{keybox}

\begin{goodbox}[title={ทางถอยที่ทดสอบแล้วว่าใช้ได้ และไม่ต้องใช้คีย์เลย}]
\ok{} ถ้าขอคีย์ไม่ทัน ใช้แฟ้ม \en{Population Estimates} แบบ CSV นิ่งแทนได้
ไม่มีระบบคีย์มาเกี่ยวข้อง

\vspace{0.3em}
\code{www2.census.gov/programs-surveys/popest/datasets/} \\
\code{2020-2024/state/asrh/sc-est2024-agesex-civ.csv} (812 KB, HTTP 200)

\vspace{0.3em}
กรอง \code{SEX=0} และ \code{AGE>=65} (ตัด \code{AGE=999} ซึ่งเป็นแถวผลรวมออก)
แล้วรวมตาม \code{NAME} ได้ครบ 50 รัฐ + DC + ผลรวมประเทศ
ทดสอบแล้วได้ยอดทั้งประเทศ \textbf{61.2 ล้านคน} ซึ่งสมเหตุสมผล
\end{goodbox}
...
Q2 & ระเบียนซ้ำ &
  ...
  ถูกรายงานซ้ำข้ามหลาย snapshot (แฟ้ม Penalties เป็นหน้าต่างหมุน 3 ปี \ok{}
  ค่าปรับหนึ่งรายการจึงปรากฏซ้ำได้ถึง 36 งวด) &
  Fact 1 ตั้งใจให้ซ้ำ (เป็นสถานะรายงวด) จึงใส่ snapshot ไว้ในคีย์หลัก
  ส่วน Fact 2 ต้องกำจัดซ้ำจริง ด้วยคีย์ \textbf{สองระบบแยกตามยุค}
  เพราะ \code{Fine ID} มีเฉพาะงวด 202606 เป็นต้นไป \ok{}
  (ดูกล่องแดงท้ายหัวข้อนี้) \\
\addlinespace
Q3 & ค่าว่าง &
...
Q4 & ค่าที่เป็นไปไม่ได้ &
  ...
  --- เกิดขึ้นจริง \textbf{128 แห่ง} (0.88\%) และค่าสูงสุดคือ
  \textbf{1414\%} ซึ่งเป็นข้อผิดพลาดแน่นอน \ok{} &
...
Q6 & รูปแบบวันที่ต่างกัน &
  ตรวจปลายทางทั้งสองฝั่งแล้ว: ทั้ง ม.ค. 2562 และ มิ.ย. 2569 ใช้
  \code{YYYY-MM-DD} เหมือนกัน \ok{} สมมติฐานเดิมว่ารูปแบบเปลี่ยน
  \textbf{ยังไม่พบหลักฐาน} แต่ยังไม่ได้ตรวจงวดกลาง &
  คงกฎเดิมไว้เป็นตาข่ายกันพลาด: แปลงด้วยรายการรูปแบบที่ระบุชัดเรียงลำดับลอง
  ถ้าไม่เข้าเลยให้บันทึกไว้ \textbf{ห้ามปล่อยให้เดารูปแบบเอง}
  เพราะสลับวันกับเดือนได้เงียบ ๆ \\
\addlinespace
Q7 & โครงสร้างคอลัมน์เปลี่ยน &
  \textbf{หนักกว่าที่คาดมาก} \ok{} ปี 2562 มี \textbf{78} คอลัมน์ ปี 2569 มี
  \textbf{99} และ \emph{ชื่อคอลัมน์เปลี่ยนทั้งระบบ}: \code{PROVNUM} $\to$
  \code{CMS Certification Number (CCN)} คอลัมน์อัตราลาออก (5) ชั่วโมงสุดสัปดาห์ (4)
  เครือ (7) พิกัด (1) ไม่มีในปี 2562 เลย &
  ...
  ใช้ Table 16 ของ \code{NH_Data_Dictionary.pdf} เป็นแหล่งอ้างอิงวันที่เปลี่ยน \\
\addlinespace
Q8 & หน่วยวัดไม่ตรงกัน &
  งวด มิ.ย. 2569 เก็บเป็น \textbf{เปอร์เซ็นต์} (มัธยฐาน 45.30 ช่วง 4--100) \ok{}
  เทียบกับปี 2562 ไม่ได้เพราะยังไม่มีคอลัมน์นี้ (ดู Q7)
  จึงยัง \textbf{เทียบข้ามยุคไม่ได้} \tbc{} &
  ตรวจจากค่ามัธยฐานของแต่ละงวด ถ้าน้อยกว่า 1.5 ให้คูณ 100
  แล้วบันทึกการแปลงลงตารางบันทึกการทำงาน กฎนี้ยังจำเป็น
  เพราะต้องรันกับงวดที่ยังไม่ได้เปิดดูอีกหลายสิบงวด \\
\bottomrule
\end{tabular}
\end{adjustbox}
...
\begin{badbox}[title={\code{Fine ID} ใช้เป็นคีย์กำจัดซ้ำได้ไม่ตลอดช่วง --- ต้องมีสองระบบ}]
เอกสารฉบับก่อนวางคีย์ของ Fact 2 ไว้ที่ (ccn, วันที่, ประเภท, fine\_id)
ตรวจจริงแล้วพบปัญหาสามชั้น \ok{}

\vspace{0.4em}
\textbf{หนึ่ง --- คอลัมน์นี้เพิ่งมี} Table 16 ของคู่มือระบุว่า
\emph{``202606 Penalties: Added Fine ID column after Penalty Type''}
แปลว่างวดก่อน มิ.ย. 2569 \textbf{ไม่มีคอลัมน์นี้เลย} ซึ่งคือเกือบทั้งโครงงาน

\vspace{0.3em}
\textbf{สอง --- ว่างทั้งหมดบนรายการระงับการจ่ายเงิน} ในงวด มิ.ย. 2569
\code{Fine ID} เป็นค่าว่างครบทั้ง \textbf{2{,}470} แถวที่เป็น Payment Denial
(ไม่ว่างเลยทั้ง 13{,}710 แถวที่เป็น Fine) จึงกำจัดซ้ำได้เฉพาะครึ่งเดียวของตาราง

\vspace{0.3em}
\textbf{สาม --- คีย์ธรรมชาติทดแทนไม่ปลอดภัย} เมื่อมี \code{Fine ID}
ค่าไม่ซ้ำเลยแม้แต่รายการเดียว (13{,}710 ค่า ต่อ 13{,}710 แถว)
แต่ถ้าใช้ (ccn, วันที่, ประเภท, จำนวนเงิน) แทน จะเหลือเพียง 13{,}424 กลุ่ม
คือ \textbf{กลืนค่าปรับจริงหายไป 286 รายการ (2.09\%) คิดเป็นเงิน 2.66 ล้านดอลลาร์ (0.58\%)}
เพราะสถานที่เดียวกันโดนปรับหลายรายการในวันเดียวด้วยจำนวนเงินเท่ากันได้จริง
เช่น CCN \code{015060} วันที่ 2025-05-09 มีค่าปรับ \$5{,}782 \emph{สามรายการ}
(Fine ID 134643, 134644, 134645)

\vspace{0.4em}
\textbf{ข้อสรุปเชิงออกแบบ:} Fact 2 ต้องมีคอลัมน์ \code{fine_id_source}
บอกว่าแถวนั้นได้คีย์มาจากไหน แล้วกำจัดซ้ำแบบสองระบบ ---
งวดตั้งแต่ 202606 ใช้ \code{Fine ID} ตรง ๆ ส่วนงวดก่อนหน้าใช้คีย์ธรรมชาติ
\textbf{บวกลำดับที่ภายในกลุ่ม} (row number ภายใน ccn+วันที่+ประเภท+จำนวนเงิน)
เพื่อไม่ให้รายการที่เหมือนกันถูกยุบรวม และต้องรายงานตัวเลข 2.09\% นี้
ไว้ในข้อจำกัดของโครงงาน
\end{badbox}
...
คอลัมน์ \code{Total Amount of Fines in Dollars} ใน ProviderInfo คือยอดรวมของ
\textbf{หน้าต่างหมุน 3 ปี} \ok{} ยืนยันสองทาง --- คู่มือระบุว่าแฟ้ม Penalties คือ
\emph{``a list of the fines\ldots in the last three years''} และเมื่อรวมยอดจากแฟ้ม
Penalties เองแล้วเทียบกลับ ได้ตรงกัน \textbf{ครบทั้ง 6{,}563 แห่ง}
ทั้งจำนวนเงินและจำนวนรายการ ส่วนวันที่ในแฟ้มก็อยู่ในช่วง
2023-06-17 ถึง 2026-05-13 เทียบกับงวด 2026-06-01 พอดี

\vspace{0.3em}
มันจึงรวมข้ามสถานที่ได้ แต่ \textbf{รวมข้ามงวดไม่ได้} เพราะจะนับซ้ำ
...
\begin{badbox}[title={หน้าต่างหมุนอันตรายกว่ายอดสะสม --- ค่าปรับ ``หายไป'' ได้เอง}]
ตอนแรกเอกสารนี้เขียนว่าคอลัมน์นั้นเป็น ``ยอดสะสม'' ซึ่งผิด
และความต่างนี้สำคัญกว่าที่เห็น

\vspace{0.3em}
ยอดสะสมจะโตขึ้นอย่างเดียว แต่ \textbf{หน้าต่างหมุนลดลงได้}
ค่าปรับที่เก่าเกิน 3 ปีจะ \emph{หลุดออกจากแฟ้มไปเงียบ ๆ} ทำให้ยอดของสถานที่หนึ่ง
ลดลงระหว่างสองงวดได้ทั้งที่ไม่มีอะไรผิดพลาด

\vspace{0.3em}
ผลที่ตามมาสองข้อ: หนึ่ง --- ห้ามตีความยอดที่ลดลงว่าเป็น ``ข้อมูลพัง''
สอง --- Fact 2 ที่สะสมค่าปรับจากทุกงวดจะเก็บประวัติได้ \textbf{ยาวกว่า}
ที่แฟ้มงวดใดงวดเดียวจะให้ได้ ซึ่งเป็นคุณค่าที่แท้จริงของการทำคลังข้อมูล
ไม่ใช่แค่คัดลอกไฟล์มาเก็บ
\end{badbox}
...
สี่ข้อแรกตรวจเสร็จแล้ว สรุปผลไว้ที่นี่ พร้อมชี้ว่าคำตอบไปเปลี่ยนอะไรในเอกสาร

\begin{table}[H]\centering
\begin{adjustbox}{max width=\linewidth}
\small
\begin{tabular}{L{0.6cm} L{4.6cm} L{6.6cm} L{4.6cm}}
\toprule
\sansall\bfseries \# & \sansall\bfseries สิ่งที่สงสัย &
\sansall\bfseries ผลการตรวจ & \sansall\bfseries กระทบอะไร \\
\midrule
1 & ยอดค่าปรับใน ProviderInfo สะสมกี่ปี &
  \textbf{3 ปีแบบหน้าต่างหมุน} ไม่ใช่สะสมตลอดกาล \ok{}
  คู่มือระบุว่า Penalties คือ \emph{``fines\ldots in the last three years''}
  และพิสูจน์ซ้ำด้วยข้อมูลจริง: ยอดใน ProviderInfo ตรงกับผลรวมจากแฟ้ม Penalties
  \textbf{ครบทั้ง 6{,}563 แห่ง (100\%)} ทั้งจำนวนเงินและจำนวนรายการ &
  เหตุผลของ Fact 2 \textbf{แข็งขึ้น} แต่ต้องเปลี่ยนถ้อยคำ (ดูภาคผนวก ข) \\
\addlinespace
2 & ปี 2562 ขาดคอลัมน์อะไรบ้าง &
  ขาดมากกว่าที่คิด: \textbf{78 vs 99 คอลัมน์} และเปลี่ยนระบบตั้งชื่อทั้งหมด
  \ok{} อัตราลาออก ชั่วโมงสุดสัปดาห์ เครือ พิกัด ไม่มีเลย &
  Q7 ยกระดับความรุนแรง และเพิ่มงานทำตารางจับคู่ชื่อคอลัมน์ \\
\addlinespace
3 & Census API ไม่คืนค่า &
  \textbf{ต้องใช้ API key} \ok{} เซิร์ฟเวอร์ตอบ 302 ไปหน้า
  \code{missing_key.html} จึงได้ body ว่างโดยไม่มี error
  ทางถอยแบบไม่ต้องใช้คีย์ทดสอบแล้วว่าใช้ได้ &
  BQ1 \textbf{ไม่ถูกบล็อกอีกต่อไป} (ดูหัวข้อแหล่งเสริม) \\
\addlinespace
4 & \code{Fine ID} ซ้ำข้าม snapshot ไหม &
  ไม่ซ้ำเลยภายในงวด (13{,}710 ค่า/13{,}710 แถว) \textbf{แต่ใช้ไม่ได้ทั้งช่วง}
  \ok{} เพราะเพิ่งมีตั้งแต่งวด 202606 และว่างทั้งหมดบนรายการระงับการจ่ายเงิน &
  คีย์ของ Fact 2 ต้องออกแบบใหม่เป็นสองระบบ (ดูกล่องแดงในภาค 3) \\
\bottomrule
\end{tabular}
\end{adjustbox}
\caption{ผลการตรวจสี่ข้อที่ค้างไว้ ทุกข้อได้คำตอบจากข้อมูลจริงหรือคู่มือฉบับทางการ ไม่มีข้อใดที่ยังต้องเดา}
\end{table}

\subsection*{ที่ยังเหลือ}
\begin{enumerate}
\item เติมชื่อสมาชิกสามคนลงหน้าปก (หมายเลขกลุ่มเติมแล้ว: กลุ่ม 05)
\item ขอ Census API key (ฟรี ใช้เวลาไม่กี่นาที) หรือยืนยันว่าจะใช้ทางถอย
\item เปิด snapshot งวดกลาง (ราวปี 2565) เพื่อปิดช่องว่างของ Q6 และ Q8
      ซึ่งตอนนี้ยืนยันได้แค่สองปลาย
\end{enumerate}
...
Total Amount of Fines in Dollars & \emph{ไม่ใช้} & ยอดรวม \textbf{หน้าต่างหมุน 3 ปี}
  ไม่ใช่ยอดสะสมตลอดกาล ใช้ Fact 2 แทน \ok{} \\
...
\item \textbf{หาสาเหตุที่ Census คืนค่าว่าง} --- ดู redirect แทนที่จะดู body \\
      \code{curl -sS -o /dev/null -w "\%\{http_code\} \%\{redirect_url\}" \textbackslash} \\
      \code{\ "https://api.census.gov/data/2023/acs/acs5?get=NAME,} \\
      \code{B01001_001E\&for=state:*"} \\
      คืน \code{302 https://api.census.gov/data/missing_key.html}
\item \textbf{ยืนยันว่ายอดค่าปรับเป็นหน้าต่าง 3 ปี} --- รวม \code{Fine Amount}
      จาก Penalties ตาม CCN แล้วเทียบกับ \code{Total Amount of Fines in Dollars}
      ใน ProviderInfo ต้องตรงกัน 100\%
\item \textbf{ยืนยันว่า Fine ID จำเป็น} --- นับ \code{nunique()} ของ Fine ID
      เทียบกับจำนวนกลุ่มของ (CCN, Penalty Date, Penalty Type, Fine Amount)
      ผลต่างคือจำนวนค่าปรับที่คีย์ธรรมชาติจะกลืนหาย
\item \textbf{เทียบโครงสร้างคอลัมน์ข้ามยุค} --- คลาย
      \code{ProviderInfo_Download.csv} จาก \code{nursing-homes_2019-01-17.zip}
      แล้วเทียบ \code{set(columns)} กับงวดปัจจุบัน
\end{enumerate}
